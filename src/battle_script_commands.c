...
static void Cmd_multihitresultmessage(void)
{
     ...
     && !(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_FOE_ENDURED))
    {
        ...
        {
            gBattleStruct->moveResultFlags[gBattlerTarget] &= ~(MOVE_RESULT_STURDIED | MOVE_RESULT_FOE_HUNG_ON | MOVE_RESULT_ANTIVIRUS_BLOCKED);
            BattleScriptCall(BattleScript_SturdiedMsg);
            return;
        }
        else if (gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_ANTIVIRUS_BLOCKED)
        {
            gBattleStruct->moveResultFlags[gBattlerTarget] &= ~(MOVE_RESULT_STURDIED | MOVE_RESULT_FOE_HUNG_ON | MOVE_RESULT_ANTIVIRUS_BLOCKED);
            BattleScriptCall(BattleScript_AntivirusProtects);
            return;
        }
...
static void Cmd_resultmessage(void)
{
    ...
    else
    {
        ...
        default:
            ...
            {
                *moveResultFlags &= ~(MOVE_RESULT_STURDIED | MOVE_RESULT_FOE_ENDURED | MOVE_RESULT_FOE_HUNG_ON | MOVE_RESULT_ANTIVIRUS_BLOCKED);
                BattleScriptCall(BattleScript_SturdiedMsg);
                return;
            }
            else if (*moveResultFlags & MOVE_RESULT_ANTIVIRUS_BLOCKED)
            {
                *moveResultFlags &= ~(MOVE_RESULT_STURDIED | MOVE_RESULT_FOE_ENDURED | MOVE_RESULT_FOE_HUNG_ON | MOVE_RESULT_ANTIVIRUS_BLOCKED);
                BattleScriptCall(BattleScript_AntivirusProtects);
                return;
...
void SetMoveEffect(enum BattlerId battlerAtk, enum BattlerId effectBattler, enum MoveEffect moveEffect, const u8 *battleScript, enum SetMoveEffectFlags effectFlags)
{
    ...
    case MOVE_EFFECT_STEAL_STATS:
        if (!IsBattlerUnaffectedByMove(effectBattler))
        {
...
static void Cmd_givecaughtmon(void)
{
    ...
    case GIVECAUGHTMON_GIVE_AND_SHOW_MSG:
    {
        ...
        {
            u16 lostItem = gBattleStruct->itemLost[B_TRAINER_OPPONENT_A][gBattlerPartyIndexes[GetCatchingBattler()]].originalItem;
...
void BS_TryActivateReceiver(void)
{
    ...
    {
        gBattleStruct->tracedAbility[gBattlerAbility] = gBattleMons[faintedBattler].ability; // re-using the variable for trace
