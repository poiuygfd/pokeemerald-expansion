...
static u32 GetSwitchinHitsToKO(s32 damageTaken, u32 battler)
{
    ...
    while (currentHP > 0)
    {
        ...
        if (damageTaken >= maxHP && startingHP == maxHP && (heldItemEffect == HOLD_EFFECT_FOCUS_SASH || (!opponentCanBreakMold && GetConfig(CONFIG_STURDY) >= GEN_5 && ability == ABILITY_STURDY) || (!opponentCanBreakMold && ability == ABILITY_ANTIVIRUS)) && hitsToKO < 1)
            currentHP = 1;
