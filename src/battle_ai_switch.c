...
static u32 GetSwitchinHitsToKO(s32 damageTaken, enum BattlerId battler, const struct IncomingHealInfo *healInfo, u32 originalHp)
{
    ...
    while (currentHP > 0)
    {
        ...
        if (damageTaken >= maxHP && startingHP == maxHP && (heldItemEffect == HOLD_EFFECT_FOCUS_SASH || (!opponentCanBreakMold && GetConfig(B_STURDY) >= GEN_5 && ability == ABILITY_STURDY) || (!opponentCanBreakMold && ability == ABILITY_ANTIVIRUS)) && hitsToKO < 1)
            currentHP = 1;
