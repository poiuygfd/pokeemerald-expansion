...
bool32 TryChangeBattleTerrain(enum BattlerId battler, enum BattleTerrain terrain)
{
    ...
    else if (terrain == B_TERRAIN_VOID)
        return FALSE;
        ...
        case ABILITY_TAG_TEAM: // Handles the transformation on using Leader Swap
            {
                ...
                && !gBattleStruct->unableToUseMove)
                {
                    ...
                    if (shouldSwap == TRUE)
                    {
                        ...
                        RecalcBattlerStats(gBattlerAttacker, mon, FALSE);
                        if (health == 0)
                            SetHealAmount(gBattlerAttacker, GetNonDynamaxMaxHP(gBattlerAttacker));
                        else if (health > gBattleMons[gBattlerAttacker].hp)
                        {
                            s32 healthDifference = health - gBattleMons[gBattlerAttacker].hp;
                            SetHealAmount(gBattlerAttacker, healthDifference);
                        }
                        else if (health < gBattleMons[gBattlerAttacker].hp)
                        {
                            s32 healthDifference = gBattleMons[gBattlerAttacker].hp - health;
                            SetPassiveDamageAmount(gBattlerAttacker, healthDifference);
                        }
                        BattleScriptCall(BattleScript_TagTeamActivates);
                        effect++;
                    }
                    else
                    { BattleScriptCall(BattleScript_EffectLeaderSwapFailed); }
                }
            }
            break;
        default:
...
bool32 IsTheVoidAffected(enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect, enum BattleTerrain currTerrain)
{
    return IsBattlerTerrainAffected(battler, ability, holdEffect, B_TERRAIN_VOID, currTerrain);
}
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    if (IsPsychicTerrainAffected(battlerAtk, ctx->abilities[battlerAtk], ctx->holdEffects[battlerAtk], ctx->terrain) && moveType == TYPE_PSYCHIC)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsTheVoidAffected(battlerAtk, ctx->abilities[battlerAtk], ctx->holdEffects[battlerAtk], ctx->terrain) && moveType == TYPE_DARK)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
