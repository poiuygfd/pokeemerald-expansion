...
bool32 IsDamageMoveUnusable(struct DamageContext *ctx)
{
    ...
    else
    {
        ...
        partnerDefAbility = aiData->abilities[GetPartnerBattler(ctx->battlerDef)];
    }
...
{
    if (!AI_BattlerAtMaxHp(battlerTarget) || (IsMultiHitMove(move) && gAiLogicData->abilities[battlerTarget]  == ABILITY_ANTIVIRUS) || gAiLogicData->abilities[battler]  == ABILITY_PARENTAL_BOND)
        return FALSE;
    ...
    if (!DoesBattlerIgnoreAbilityChecks(battler, gAiLogicData->abilities[battler], move))
    {
        if (GetConfig(B_STURDY) >= GEN_5 && gAiLogicData->abilities[battlerTarget] == ABILITY_STURDY)
            return TRUE;
        if (gAiLogicData->abilities[battlerTarget] == ABILITY_ANTIVIRUS)
            return TRUE;
...
{
    if (IsAiBattlerAware(battler) || IsAiBattlerAware(GetPartnerBattler(battler)))
...
{
    enum BattlerId partnerBattler = GetPartnerBattler(battlerId);
...
bool32 SideHasMoveCategory(enum BattlerId battlerId, enum DamageCategory category)
{
    ...
    {
        if (HasMoveWithCategory(battlerId, category) || HasMoveWithCategory(GetPartnerBattler(battlerId), category))
            return TRUE;
    }
    else
    {
