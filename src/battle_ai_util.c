...
{
    enum BattleMoveEffects effect = GetMoveEffect(move);
    if (!AI_BattlerAtMaxHp(battlerTarget) || (effect == EFFECT_MULTI_HIT && gAiLogicData->abilities[battlerTarget] != ABILITY_ANTIVIRUS) || gAiLogicData->abilities[battler]  == ABILITY_PARENTAL_BOND)
        return FALSE;
